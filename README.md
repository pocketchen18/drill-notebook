...
| 功能 | 说明 |
|---|---|
| **背知识点重构** | 树状大纲阅读与卡片工作区：左侧多级大纲导航（支持搜索过滤、按标签筛选） + 右侧卡片工作区；点卡片或按全屏按钮进入 `KnowledgeFullCardView` 全屏阅读视图（支持大纲拖拽调宽与阈值自动折叠、键盘快捷翻页、章节/整库/单卡总结与还原、修改/删除按钮，`Escape` 退出） |
...
2. **多级大纲浏览**：点击章节目录查看递归章节内容，点击最顶层总根目录查看全库总览。支持按关键词搜索和按标签快速过滤。  
3. **卡片阅读与切换**：
