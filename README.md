...
| 版本线索 | `git log`：`v0.3` PDF/导出/高级挑题；`v0.4` 记忆曲线 + 日历 + AI 多会话；`v0.5` AI 总结知识点 + 背知识点重构 + 笔记本混合检索（BM25 + 可选向量 RAG）；`v0.5.1` 会话内短周期记忆曲线（循环背诵全流程重构）+ 题库选题继承 + 会话↔日历联动（顽固项加练、评分最终态、日历实时今天）；`v0.5.2` 界面状态记忆（启动回到上次页面 + 各页选择/切换/筛选持久化）；`v0.5.3` 常规设置完善（主题跟随系统 + AI 悬浮球开关 + 自定义快捷键：全局 + 页面内）；`v0.6`（规划）AI 跨会话记忆 |
