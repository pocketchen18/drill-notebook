...
export function KnowledgePointPage(): JSX.Element {
  ...
  const saveMutation = useMutation({ mutationFn: () => editing ? put(`/api/knowledge-points/${editing.id}`, { title, content, category: category || null, tags: tagText.split(/[,，]/).map((item) => item.trim()).filter(Boolean), questionIds: linkedQuestionIds }) : post('/api/knowledge-points', { bankId, title, content, category: category || null, tags: tagText.split(/[,，]/).map((item) => item.trim()).filter(Boolean), questionIds: linkedQuestionIds }), onSuccess: () => { refresh(); setEditorVisible(false); Message.success(editing ? '知识点已更新' : '知识点已创建'); }, onError: (error) => Message.error(error.message) });
  const deleteMutation = useMutation({ mutationFn: (id: number) => del(`/api/knowledge-points/${id}`), onSuccess: () => { refresh(); Message.success('知识点已删除'); }, onError: (error) => Message.error(error.message) });
  const importMutation = useMutation({ mutationFn: (markdown: string) => post<{ imported: number; failed: number; errors: string[] }>('/api/knowledge-points/import/markdown', { bankId, content: markdown }), onSuccess: (result) => { refresh(); Message.success(`已导入 ${result.imported} 个知识点`); if (result.errors.length) Message.warning(result.errors.slice(0, 2).join('；')); }, onError: (error) => Message.error(error.message) });
  ...
  const importMarkdown = async (): Promise<void> => { if (window.api) { const result = await window.api.dialog.openTextFile(); if (!result.canceled && result.content !== undefined) importMutation.mutate(result.content); } else fallbackFile.current?.click(); };
  const jump = (next: number): void => { setIndex(next); setRevealed(false); };

  return <main className="page"><input hidden ref={fallbackFile} type="file" accept=".md,.markdown,.txt" onChange={(event) => { const file = event.target.files?.[0]; if (file) void file.text().then((value) => importMutation.mutate(value)); event.target.value = ''; }} />
    <div className="page-heading"><div><h1>背知识点</h1><p>建立真正的知识点库，关联题目，并按分类、标签或自定义顺序反复记忆。</p></div><Space>{sessionIds ? <Button icon={<RotateCcw size={16} />} onClick={() => setSessionIds(undefined)}>返回知识库</Button> : <><Button icon={<FileUp size={16} />} loading={importMutation.isPending} onClick={() => void importMarkdown()}>导入 Markdown</Button><Button type="primary" icon={<Plus size={16} />} onClick={() => openEditor()}>新建知识点</Button></>}</Space></div>
