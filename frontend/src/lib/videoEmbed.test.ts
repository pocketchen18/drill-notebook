...
describe('resolveEmbedUrl', () => {
  ...
  it('resolves bilibili BV URL', () => {
    ...
    expect(result.embedUrl).toBe('https://player.bilibili.com/player.html?bvid=BV1xx411c7mD&page=1&autoplay=0&high_quality=1');
  });

  it('resolves bilibili AV URL', () => {
    const result = resolveEmbedUrl('https://www.bilibili.com/video/av170001');
    expect(result.platform).toBe('bilibili');
    expect(result.embedUrl).toBe('https://player.bilibili.com/player.html?aid=170001&page=1&autoplay=0&high_quality=1');
  });
